...
import customeRoute from '../fixtures/routes.json'
import customeTrip from '../fixtures/routes-4-trips.json'
import customeVehicles from '../fixtures/vehicle_positions.json'
import customeStop from '../fixtures/stop.json'
...
function Main() {
  ...
  console.log(customeVehicles);
  useEffect(() => {
    getJSON(`${GTFS_BASE_URL}/routes.json`)
      .then((r) =>{
         setRouteData(r.data)
         setRouteData(customeRoute.data)
    ...
    //   .then(() => {
    //     // setTripUpdates(data)
    //     setTripUpdates(customeTrip)
    //   })
    //   .then(() => setTripUpdateLoaded(true))
    //   .catch((error) => setDataFetchError(error));
      setTripUpdates(customeTrip)
      setTripUpdateLoaded(true)
    getJSON(`${GTFS_BASE_URL}/realtime/vehicle_positions.json`)
      .then(() => setVehicleMarkers(customeVehicles))
    ...
    const refreshPositionsInterval = setInterval(() => {
      ...
      getJSON(`${GTFS_BASE_URL}/realtime/vehicle_positions.json`)
        .then(() => {
          // setVehicleMarkers(data)
          setVehicleMarkers(customeVehicles)
    ...
    const refreshBCycleStatus = setInterval(() => {
      ...
      getJSON(`${GBFS_BASE_URL}/station_status.json`)
        .then(() => setBCycleStationStatusData(customeStop.data.stations));
    }, REFRESH_GBFS_TTL);
